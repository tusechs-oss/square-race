...
func _enter_tree() -> void:
	...
	# Start connection
	var override_url := ""
	if ProjectSettings.has_setting("plugins/godot_mcp/server_url"):
		override_url = str(ProjectSettings.get_setting("plugins/godot_mcp/server_url"))
	elif OS.has_environment("GODOT_MCP_URL"):
		override_url = OS.get_environment("GODOT_MCP_URL")
	if override_url != "":
		_mcp_client.connect_to_server(override_url)
	else:
